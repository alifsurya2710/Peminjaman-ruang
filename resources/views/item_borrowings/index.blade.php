            ...
            <div class="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 class="text-3xl font-extrabold tracking-tight mb-2 flex items-center gap-3">
                        <span
                            class="w-10 h-10 bg-white/15 rounded-xl flex items-center justify-center border border-white/20">
                            <i class="fas fa-people-carry-box text-white"></i>
                        </span>
                        Peminjaman Barang
                    </h1>
                    <p class="text-white/85 font-medium max-w-md">Kelola & setujui peminjaman barang.</p>
                </div>
                <a href="/item_borrowings/create"
                    class="group flex items-center gap-3 px-6 py-3 bg-primary-600 hover:bg-primary-500 text-white rounded-2xl transition-all duration-300 font-bold text-sm shadow-lg shadow-primary-900/20">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 border-b border-slate-100">
                            <th class="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] w-14">No
                            </th>
                            <th class="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Peminjam
                            </th>
                            <th class="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Kelas &
                                Jurusan</th>
                            <th class="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Barang &
                                Jumlah</th>
                            <th class="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Tanggal
                            </th>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        @forelse($borrowings as $borrowing)
                            <tr
                                class="hover:bg-indigo-50/20 transition-all duration-300 group {{ $borrowing->status === 'pending' ? 'bg-amber-50/30' : '' }}">
                                <td class="px-6 py-5">
                                    <span
                                        class="text-xs font-black text-slate-300 group-hover:text-indigo-400 transition-colors">
                                ...
                                <td class="px-6 py-5">
                                    <div class="flex flex-col gap-1.5">
                                        @if($borrowing->class_name)
                                            <span
                                                class="inline-flex items-center gap-1.5 text-xs font-bold text-indigo-700 bg-indigo-50 border border-indigo-100 px-2.5 py-1 rounded-lg w-fit">
                                        ...
                                        @if($borrowing->department)
                                            <span
                                                class="inline-flex items-center gap-1.5 text-xs font-bold text-violet-700 bg-violet-50 border border-violet-100 px-2.5 py-1 rounded-lg w-fit">
                                ...
                                <td class="px-6 py-5">
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <span
                                            class="text-xs font-extrabold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-lg w-fit">{{ $borrowing->amount }}
                                            unit</span>
                                    </div>
                                </td>
                                <td class="px-6 py-5">
                                ...
                                <td class="px-6 py-5">
                                    <span
                                        class="px-3 py-1.5 text-xs font-bold rounded-xl {{ $borrowing->getBadgeClasses() }} border">
                            ...
                            <tr>
                                <td colspan="7" class="px-8 py-20 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        <div
                                            class="w-20 h-20 rounded-3xl bg-slate-50 text-slate-200 flex items-center justify-center mb-6 ring-8 ring-slate-50/50">
                                            <i class="fas fa-folder-open text-3xl"></i>
                                        </div>
                                        <h3 class="text-lg font-bold text-slate-800">Data Peminjaman Kosong</h3>
                                        <p class="text-slate-400 max-w-xs mx-auto mt-1">Belum ada transaksi peminjaman barang.
                                        </p>
                                    </div>
                                </td>
                            </tr>
