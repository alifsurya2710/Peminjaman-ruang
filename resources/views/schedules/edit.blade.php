...
@section('content')
    <div class="max-w-4xl">
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Edit Jadwal Pembelajaran</h2>
                <p class="text-slate-500 text-sm">Perbarui informasi jadwal rutin untuk ruangan <span class="text-primary-600 font-bold">{{ $schedule->room->name }}</span>.</p>
            </div>
        </div>

        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
            <div class="p-8">
                <form action="/schedules/{{ $schedule->id }}" method="POST" class="space-y-8">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div class="space-y-2">
                                    ...
                                    <select class="block w-full px-4 py-3 bg-slate-50/50 border border-slate-200 rounded-2xl text-slate-700 font-medium focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all @error('block') border-rose-400 ring-rose-400/10 @enderror" id="block" name="block" required>
                                        <option value="1" {{ old('block', $schedule->block) == '1' ? 'selected' : '' }}>Blok 1</option>
                                        <option value="2" {{ old('block', $schedule->block) == '2' ? 'selected' : '' }}>Blok 2</option>
                                    </select>
                                </div>
                                <div class="space-y-2">
                                    ...
                                    <select class="block w-full px-4 py-3 bg-slate-50/50 border border-slate-200 rounded-2xl text-slate-700 font-medium focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all @error('semester') border-rose-400 ring-rose-400/10 @enderror" id="semester" name="semester" required>
                                        <option value="1" {{ old('semester', $schedule->semester) == '1' ? 'selected' : '' }}>Ganjil (1)</option>
                                        <option value="2" {{ old('semester', $schedule->semester) == '2' ? 'selected' : '' }}>Genap (2)</option>
                                    </select>
                                </div>
                            </div>
