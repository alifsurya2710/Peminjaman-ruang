...
<div class="max-w-3xl mx-auto">
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Tambah Background Baru</h2>
            <p class="text-slate-500 text-sm">Upload gambar kustom untuk latar belakang halaman autentikasi.</p>
        </div>
    </div>

    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-8 md:p-12">
            <form action="{{ route('auth-backgrounds.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                @csrf

                <!-- Name Input -->
                <div class="space-y-2">
                    <label class="text-sm font-bold text-slate-700 ml-1" for="name">Nama Background</label>
                    <input type="text" id="name" name="name" value="{{ old('name') }}" required class="block w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-700 font-medium placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all" placeholder="Contoh: Gedung SMKN 1 Katapang Depan">
                    @error('name')
                ...
                <div class="space-y-2">
                    <label class="text-sm font-bold text-slate-700 ml-1">File Gambar Latar Belakang</label>
                    <div x-data="{ preview: null }" class="relative">
                        <input type="file" id="image" name="image" required accept="image/*" @change="
                        ...
                        <div x-show="preview" class="relative rounded-3xl overflow-hidden shadow-lg border border-slate-100 bg-white p-2" x-cloak>
                            ...
                            <button type="button" @click="preview = null; document.getElementById('image').value = ''" class="absolute top-4 right-4 w-10 h-10 rounded-full bg-rose-500 text-white flex items-center justify-center hover:bg-rose-600 shadow-lg">
                ...
                <div class="pt-6 flex gap-4">
                    <button type="submit" class="flex-1 py-4 px-8 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-2xl shadow-lg shadow-primary-200 transition-all hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-3 cursor-pointer">
                        <i class="fas fa-check"></i> Simpan Background
                    </button>
