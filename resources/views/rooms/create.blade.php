...
@section('content')
    <div class="max-w-3xl">
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Tambah Ruangan Baru</h2>
                <p class="text-slate-500 text-sm">Lengkapi formulir di bawah untuk menambahkan data ruangan.</p>
            </div>
        </div>

        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
            <div class="p-8">
                <form action="/rooms" method="POST" class="space-y-6" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" class="block w-full px-4 py-3 bg-slate-50/50 border border-slate-200 rounded-2xl text-slate-700 font-medium placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all @error('name') border-rose-400 ring-rose-400/10 @enderror" id="name" name="name" value="{{ old('name') }}" placeholder="Contoh: Lab RPL 1" required>
                            @error('name')
                                <p class="text-xs font-bold text-rose-500 ml-1 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" class="block w-full pl-11 pr-4 py-3 bg-slate-50/50 border border-slate-200 rounded-2xl text-slate-700 font-medium focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all @error('capacity') border-rose-400 ring-rose-400/10 @enderror" id="capacity" name="capacity" value="{{ old('capacity') }}" placeholder="0" required>
                        </div>
                        @error('capacity')
                            <p class="text-xs font-bold text-rose-500 ml-1 mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea class="block w-full px-4 py-3 bg-slate-50/50 border border-slate-200 rounded-2xl text-slate-700 font-medium placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all @error('description') border-rose-400 ring-rose-400/10 @enderror" id="description" name="description" rows="4" placeholder="Jelaskan fasilitas atau kondisi ruangan...">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-xs font-bold text-rose-500 ml-1 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <p class="text-[10px] text-slate-400 ml-1 mt-1 font-medium italic">*Format: JPG, PNG, WEBP (Maks. 2MB)</p>
                        @error('image')
                            <p class="text-xs font-bold text-rose-500 ml-1 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
